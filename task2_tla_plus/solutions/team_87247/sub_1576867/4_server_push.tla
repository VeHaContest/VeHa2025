...
Write(user, line_num) ==
    ...
    \* /\ Modified(user) = FALSE     Enabling conditions should be on top  
    /\ UNCHANGED <<log>>
...
AbleToPush(user) ==
    (* VVVV     INSERT YOU CODE HERE     VVVV *)
    /\ Modified(user) = FALSE \** (Modified(user) = TRUE) <=> changes are not commited
...
LogAlwaysGrows ==
    \** \A u \in PARTICIPANTS: <>(Len(log[u]) = MAX_LOG_SIZE)
    \* Since the on-push conflict will eventually occur, servers log wont grow 
