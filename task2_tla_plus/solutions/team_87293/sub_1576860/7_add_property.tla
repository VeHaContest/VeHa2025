----MODULE 7_add_property----
EXTENDS Naturals, Sequences, FiniteSets

(*    ОПИСАНИЕ СПЕЦИФИКАЦИИ №7    *)

(* В спецификацию добавлена доп. структура данных merge
   для отслеживания действий Merge,
   merge[user] = TRUE тогда и только тогда,
   когда user совершает действие Merge.
   Структура данных потребуется
   только при проверке последнего свойства.
 *)
...
Init ==
    ...
    /\ merge = [user \in USERS |-> FALSE]

ResetMergeStruct ==
    merge' = [u \in USERS |-> FALSE]

Next ==
    \/ WriteAction /\ ResetMergeStruct
    \/ CommitAction /\ ResetMergeStruct
    \/ PushAction /\ ResetMergeStruct
    \/ PullAction /\ ResetMergeStruct
    \/ MergeAction
...
\* ДОБАВЛЕН ИНВАРИАНТ LogServerIncludeLogUserAfterMerge:
\* После слияния (Merge) лог сервера включен в лог пользователя.
LogServerIncludeLogUserAfterMerge ==
    \A user \in USERS:
        merge[user] => Include(log[SERVER], log[user])

\* Не удалось устранить предупреждение от TLC по поводу
\* доп. переменной merge, но инваринт рабочий
\* и позволяет искать контрпримеры.
