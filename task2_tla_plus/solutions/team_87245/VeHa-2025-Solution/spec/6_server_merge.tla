...
AbleToPush(user) ==
    ...
    /\ \A i \in 1..Len(log[SERVER]): log[SERVER][i] = log[user][i]
...
AbleToPull(user) ==
    ...
    /\ \A i \in 1..Len(log[user]): log[user][i] = log[SERVER][i]
...
DivergentIndex(user) ==
    ...
    CHOOSE i \in DOMAIN log[SERVER]:
        /\ i \in DOMAIN log[user]
        /\ \A j \in DOMAIN log[SERVER] \cap DOMAIN log[user]: j < i => log[user][j] = log[SERVER][j]
...
ChangedEntries(user) ==
    ...
    LET di == DivergentIndex(user) IN
        [ i \in di..Len(log[user]) |-> ChangedLineAndValue(log[user][i-1].file, log[user][i].file) ]
...
ApplyDivergence(user, new_log, changed_entries) ==
    ...
    IF Len(changed_entries) = 0 THEN new_log
    ELSE
      LET e == Head(changed_entries) IN
      LET base == new_log[Len(new_log)].file IN
      LET newFile == [base EXCEPT ![e.line_num] = e.value] IN
        ApplyDivergence(
            user,
            Append(new_log, [user |-> user, file |-> newFile]),
...
ServerFileMatchesLog ==
    file[SERVER] = LastLogEntry(SERVER).file

FirstEntryAuthoredByServer ==
    /\ \A p \in PARTICIPANTS: 1 \in DOMAIN log[p]
    /\ \A p \in PARTICIPANTS: log[p][1].user = SERVER
