...
\<comment> \<open>FINAL VERSION WITH ALL PROOFS INCLUDED\<close>
...
next
  ...
  next
    ...
    then have \<open>length (pick (Suc k) (y # ys)) = Suc (min k (length ys))\<close> using IH1 by simp
...
lemma pick_items_0: \<open>i < length (pick n xs) \<Longrightarrow> (pick n xs) ! i = xs ! i\<close>
proof (induction n arbitrary: i xs)
  case 0
  assume \<open>i < length (pick 0 xs)\<close>
  then have \<open>i < 0\<close> by simp
  then have False by simp
  then show \<open>i < length (pick 0 xs) \<Longrightarrow> (pick 0 xs) ! i = xs ! i\<close> by simp
next
  case (Suc k)
  assume IH1: \<open>\<And>(i :: nat) (xs :: 'a list). i < length (pick k xs) \<Longrightarrow> (pick k xs) ! i = xs ! i\<close>
  then show \<open>i < length (pick (Suc k) xs) \<Longrightarrow> (pick (Suc k) xs) ! i = xs ! i\<close>
  proof (induction xs arbitrary: i)
    case Nil
    show \<open>i < length (pick (Suc k) Nil) \<Longrightarrow> (pick (Suc k) Nil) ! i = Nil ! i\<close> by simp
  next
    case (Cons y ys)
    then show \<open>i < length (pick (Suc k) (y # ys)) \<Longrightarrow> (pick (Suc k) (y # ys)) ! i = (y # ys) ! i\<close>
    proof (induction i)
      case 0
      have \<open>(pick (Suc k) (y # ys)) ! 0 = (y # (pick k ys)) ! 0\<close> by simp
      then have P: \<open>(pick (Suc k) (y # ys)) ! 0 = y\<close> by simp
      have \<open>(y # ys) ! 0 = y\<close> by simp
      then show \<open>(pick (Suc k) (y # ys)) ! 0 = (y # ys) ! 0\<close> by simp
    next
      case (Suc m)
      have pos: \<open>0 < Suc m\<close> by simp
      assume H: \<open>(Suc m) < length (pick (Suc k) (y # ys))\<close>
      have \<open>length (pick (Suc k) (y # ys)) = min (Suc k) (length (y # ys))\<close> using pick_length by simp
      then have R: \<open>(Suc m) < min (Suc k) (length (y # ys))\<close> using H by simp
      then have sm_lt: \<open>(Suc m) < length (y # ys)\<close> by simp
      with R have \<open>(Suc m) < Suc (min k (length ys))\<close> by simp
      then have P: \<open>m < min k (length ys)\<close> by simp
      moreover have \<open>length (pick k ys) = min k (length ys)\<close> using pick_length by blast
      then have m_lt: \<open>m < length (pick k ys)\<close> using P by simp
      have \<open>(pick (Suc k) (y # ys)) ! (Suc m) = (y # (pick k ys)) ! (Suc m)\<close> by simp
      then have \<open>(pick (Suc k) (y # ys)) ! (Suc m) = tl (y # (pick k ys)) ! ((Suc m) - 1)\<close> using H pos by simp
      then have \<open>(pick (Suc k) (y # ys)) ! (Suc m) = (pick k ys) ! m\<close> by simp
      then have \<open>(pick (Suc k) (y # ys)) ! (Suc m) = ys ! m\<close> using m_lt IH1 by simp
      then have \<open>(pick (Suc k) (y # ys)) ! (Suc m) = tl (y # ys) ! ((Suc m) - 1)\<close> by simp
      then show \<open>(pick (Suc k) (y # ys)) ! (Suc m) = (y # ys) ! (Suc m)\<close> using sm_lt pos by simp
    qed
  qed
qed
