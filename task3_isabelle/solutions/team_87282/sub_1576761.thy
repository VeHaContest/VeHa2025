...
primrec pick :: \<open>nat \<Rightarrow> 'a list \<Rightarrow> 'a list\<close> where
  ...
  \<open>pick (Suc n) xs = (case xs of [] \<Rightarrow> [] | a # list \<Rightarrow> a # (pick n list))\<close>
...
lemma pick_length: \<open>length (pick n xs) = min n (length xs)\<close>
  by (induct n arbitrary: xs) (simp_all split: list.split)
...
next
  case (Suc n)
  {
    fix i j y ys assume "i < length (pick (Suc n) xs)"  "i = Suc j" "xs = y # ys"
    with Suc.hyps[of ys] have "pick (Suc n) xs ! i = xs ! i" unfolding pick_length
      by (simp add: nth.nth_Cons)
  }
  thus ?case by (simp add: nth.nth_Cons split: list.split nat.split)
qed
...
lemma conss_length_isar: \<open>length (conss xs ys) = min (length xs) (length ys)\<close>
  sorry
